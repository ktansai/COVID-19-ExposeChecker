  ...
  <v-container class="text-center px-8">
      ...
      <v-expansion-panels accordion multiple v-model="panel">
        <v-expansion-panel id="notes">
          ...
          <v-expansion-panel-content class="text-left">
            <ul>
              <li>
                iOS/Androidで収集されている COVID-19
                接触のログデータの中で、COCOAの新規陽性登録者の端末の近くにいた記録があるかを確認するツールです。
              </li>
              <br />
              <li>
                具体的には、接触ログデータの中のMatchCount(iOS)/matchesCount(Android)の項目が0ではない値(接触が疑われるデータ)を抽出するだけのツールです
              </li>
              <br />
              <li>
              ...
              <br />
              <li>
                BluetoothがOFFに設定されている間は、OSが接触検知を行うことができないためご注意ください(<a
            ...
            <ul>
              <li>
                本ツールの利用には、新型コロナウイルス接触確認アプリCOCOAのインストールが必要です。COCOAに関しては、<a
              ...
              <br />
              <li>
                接触があった日時は、Hash値を用いて、サイト下部の<a
                  href="https://cacaotest.sakura.ne.jp/"
                  target="_blank"
                  >外部ページ</a
                >にて24時間の範囲で絞り込み可能です。(iOSのみ)
              </li>
              <br />
              <li>
                接触通知のログデータ内部のTimestampは、接触のあった日時ではなく、COCOAが新規陽性登録者のリストをダウンロードし、スマホ内で照合を行った日時のようです。つまりこのTimestampより前に接触の可能性があることのみ分かります。<br /><br />
              ...
              <br />
              <li>
                現在、このTimestampが誤解を招くため、結果表示から意図的に削除させて頂いています。表示方法などを検討し将来的に表示する予定です。
              </li>
              <br />
              <li>
        ...
        <v-expansion-panel>
          ...
          <v-expansion-panel-content>
            <p class="text-left">
              Step1. Androidの設定アプリから
              接触通知ログをGoogleDriveまたはローカルに保存する<br />
            </p>
    ...
    <v-row class="justify-center mt-16" v-if="resultText.length > 0">
      <div>
        ...
        <p class="text-caption my-4">
          <span class="d-inline-block"
            >スマートフォン内のログは14日間しか残りません。</span
          ><br />
          <span class="d-inline-block"
            >上のボタンを押すとGoogleカレンダーに</span
          >
    ...
    <v-col cols="12" class="my-8">
      ...
      <p>
        上記Hash値をコピーし、
        <a
          href="https://datastudio.google.com/u/0/reporting/069598a2-3f01-4b51-b023-cdb478992182"
          target="_blank"
          >接触日シート別冊(別サイト)</a
        ><br />
        または、
        <a href="https://cacaotest.sakura.ne.jp/" target="_blank"
          >CACAOtest(別サイト)</a
        >
  ...
  methods: {
    checkJson: function () {
      function checkCOCOA2iOS(matchedExposures) {
        let result = false;
        matchedExposures.flatMap((checkItem) =>
          checkItem.Files.forEach((file) => {
            if (!("MatchCount" in file)) {
              result = true;
            }
          })
        );
      ...
      try {
        ...
        if ("ExposureChecks" in exposeData) {
          ...
          switch (exportVersion) {
            case 1:
              matchedExposures = exposeData.ExposureChecks.filter(
                (checkItem) => checkItem.MatchCount > 0
              ).map((exposure) => {
            ...
            default:
              ...
              matchedExposures = exposeData.ExposureChecks.flatMap(
                (checkItem) =>
                  checkItem.Files.filter((file) => file.MatchCount > 0)
              ).map((exposure) => {
                delete exposure.Timestamp;
                return exposure;
              });
        ...
        if (matchedExposures.length === 0) {
          ...
          this.resultJsonText = matchedExposures
